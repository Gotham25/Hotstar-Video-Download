...
function populateCompletionProgress(data) {
    ...
    if (data.indexOf("Duration") > -1) {
        ...
        totalTimeInMilliSec = getMilliseconds(totalDuration);
    } else {
        var matchSize = Math.max(timeMatches.length, sizeMatches.length);
        for (var i = 0; i < matchSize; i++) {
            cProgressOptions.percent = Math.round(((getMilliseconds(timeMatches[i]) / totalTimeInMilliSec).toFixed(2) * 100));
